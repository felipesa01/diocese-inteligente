...
export class OnClickComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  openMenuFeatures(): void {
    ...
    this.geoService.openMenuOnClickComponent(dialogConfig);
  ...
  // async getMappedMenu(list: TileLayer<any>[] | ImageLayer<any>[], coord: number[], resolution: number) {
  ...
  //         var layerSource = item['fonteGS_back'] ? item['fonteGS_back'] : item['fonteGS_front'];
  //         var col: string = item['fk_name'] ? item['fk_name'] : item['pk_name']
  //         var values: string[] = item['fk_name'] ? features.map(e => e.properties[item['fk_name']]) : features.map(e => e.properties[item['pk_name']])
  //         var geoms = await lastValueFrom(this.apiConection.getMultipleFeaturesGeomFromGS(layerSource, col, values));

  //         // console.log('Antes de chamar')
  //         // var result = await lastValueFrom(this.apiConection.getMultipleFeaturesGeomFromGS(layerSource, col, values));
  //         // var geoms = result.data;
  //         // item['fonteGS_front'] = `${item['fonteGS_front'].split(':')[0]}:${result.new_layer_name}`
  ...
  //             var coords = geoms.features.filter(e => e.properties[columnToSearch] == f.properties[columnToSearch])[0].geometry['coordinates']
  //             mappedMenu.push({
  ...
  //               geomFeature: new Feature({ geometry: new this.geoService.geomTypes[f.geometry['type']](coords) }),
...
// Usage:
// const myFeature = ... // Your feature object
// const layer = findLayerOfFeature(map, myFeature);
// if (layer) {
//   console.log('Feature found in layer:', layer);
// }
    ...
    if (list.length > 0) {
      ...
      list.map(item => {
        if (item) {
          ...
          let layer = this.geoService.findLayerOfFeature(item);
          // console.log('layer', layer)
          // console.log('aqui', item.get(layer.get('pk_name')))
  ...
  closeMyMenu() {
    ...
    this.geoService.closeOnClickComponent();
  }
