...
export class TimelineToolbarComponent implements OnInit {
  ...
  async getAttributes(layerSource: string) {
    ...
    var result = await lastValueFrom(this.apiConection.getColumnsApelidoFromPG_beta(this.layerId.toString()));
    var apelidos = Object.keys(result).reduce((obj, key) => { obj[key] = result[key].apelido; return obj;}, {});
  ...
  async changeSourceLayer() {
    ...
    if (this.layer instanceof TileLayer || this.layer instanceof ImageLayer) {
  ...
  close() {
    this.geoservice.resetTimelineLayer(this.layer as TileLayer<any>)
    this.geoservice.timelineIsOpen$.next({ layerId: this.layer.get('id') , on: false });
  }
