...
export class MainLayerListComponent implements OnInit, OnDestroy {
  ...
  constructor(public layersService: LayersManagementService, public viewContainerRef: ViewContainerRef, public dir: Directionality, public geoService: GeoService, protected changeDetectorRef: ChangeDetectorRef, private dialog: MatDialog, public dialogRef: MatDialogRef<SecLayerListComponent>) {
    ...
    this.resizeSidebarSub = this.geoService.resizeSidebar.subscribe((e) => {
      ...
      this.position = { left: e + 10 + 'px', top: '8vh' }
  ...
  drop(event: CdkDragDrop<listItem[], listItem[]>) {
    if (event.previousContainer === event.container) {
      if (event.container.id != 'secLayerList') {
        ...
        this.geoService.adjustZIndex(event.container.data);
      } else return;
    }
  }
  ...
  closeLegend(item: listItem) {
    this.geoService.toggleLegend.next({ id: item.id, open: false });
  }

  allLegendOpened = false;
  toggleAllLegend() {
    this.allLegendOpened = !this.allLegendOpened

    this.geoService.mainLayers.forEach(e => {
      this.geoService.toggleLegend.next({ id: e.id, open: this.allLegendOpened })
    })
  ...
  toggleAllVisible() {
    ...
    this.geoService.mainLayers.forEach(e => {
      this.geoService.toggleVisible.next({ id: e.id, visible: this.allLayersVisible })
    })

  }

  allLayersClicavel = true;
  toggleAllClicavel() {
    this.allLayersClicavel = !this.allLayersClicavel;

    this.geoService.mainLayers.forEach(e => {
      this.geoService.toggleClicavel.next({ id: e.id, clicavel: this.allLayersClicavel })
    })
  }


  removeAllLayers() {
    this.geoService.removeAllLayersFromMapAndSidebar()
  }
