...
export class LayerItemComponent implements OnInit, OnChanges, OnDestroy {
  ...
  toggleLegendSub: Subscription;
  toggleVisibleSub: Subscription;
  toggleClicavelSub: Subscription;
  ...
  constructor(private geoService: GeoService, private _elementRef: ElementRef, private renderer2: Renderer2, private apiConection: ApisConectionService, private dialog: MatDialog) {
    ...
    this.toggleLegendSub = this.geoService.toggleLegend.subscribe(e => {
      if (e && e.id == this.id) {
        if (e.open) this.legendIsOpened = false;

        this.toggleLegend();
      }
    })

    this.toggleVisibleSub = this.geoService.toggleVisible.subscribe(e => {
      if (e && e.id == this.id) this.apply(e.visible);
    })


    this.toggleClicavelSub = this.geoService.toggleClicavel.subscribe(e => {
      if (e && e.id == this.id) {
        if (e.clicavel) this.item.layer.set('clicavel', false);
        this.toggleLayerClickable();
      }
    })
  ...
  ngOnDestroy(): void {
    ...
    this.toggleLegendSub.unsubscribe();
    this.isFilteredSub.unsubscribe();
    this.toggleVisibleSub.unsubscribe();
    this.toggleClicavelSub.unsubscribe();
  }
  ...
  getFunctionActivation(id: number) {
    // Desativar Enquadrar para todos
  ...
  ngOnInit() {
    this.initClicavel = this.item.layer.get('clicavel');
  ...
  async toggleLegend() {
    ...
    var legendSpinner = element.getElementsByClassName('legend-spinner')[0];
    ...
    if (this.legendIsOpened) {
      this.renderer2.setStyle(legendSpinner, "display", 'flex');
      ...
      if (!this.legendImageBLOB) this.legendImageBLOB = await lastValueFrom(this.apiConection.getLegendFileFromGS(urlImage));
      this.renderer2.setAttribute(legendImg, "src", this.legendImageBLOB as string)

      this.renderer2.setStyle(legendSpinner, "display", 'none');
    }
  }
  ...
  toggleLayerClickable() {
    if (!this.initClicavel) return;
    this.item.layer.get('clicavel') ? this.item.layer.set('clicavel', false) : this.item.layer.set('clicavel', true);
  }
