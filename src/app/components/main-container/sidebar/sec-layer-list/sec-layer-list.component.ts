...
export class SecLayerListComponent {
  ...
  isZoomed(node: TreeItemNode) {
    return this.geoService.igrejaEmFoco == node.item.id
  }
  ...
  zoomToFeature(node: TreeItemNode) {
    this.geoService.map.getView().fit(node.item.feature.getGeometry().getExtent(), { padding: [150, 150, 150, 200], duration: 500, maxZoom: 19 });
    setTimeout(() => {
      this.geoService.igrejaEmFoco = node.item.id
    }, 10)
  }
  ...
  openInfo(node: TreeItemNode) {

    let obj = {
      layerId: node.item.layer.get('id'),
      layerName: node.item.layer.get('name'),
      fonteGS_front: node.item.layer.get('fonteGS_front'),
      fonteGS_back: node.item.layer.get('fonteGS_back'),
      pk_name: node.item.layer.get('pk_name'),
      pk_value: node.item.feature.get(node.item.layer.get('pk_name')),
      geomFeature: node.item.feature,
      nameFeature: node.item.feature.get(node.item.layer.get('feat_apelido')),
      cluster_mode: '',
      cluster_name: ''
    }

    if (this.geoService.generalInfoDialog instanceof MatDialogRef) {
      this.geoService.setCloseLayersModals('info' as 'info' | 'search' | 'filter' | 'edit', node.item.layer.get('id'), node.item.feature.get(node.item.layer.get('pk_name'))?.toString())
      this.geoService.generalInfoDialog?.close();
    }
    this.geoService.openGeneralFeatureInfo(obj);
