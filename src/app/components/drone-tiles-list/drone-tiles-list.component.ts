...
export class DroneTilesListComponent implements OnInit, OnDestroy {
  ...
  async prepareTable() {
    var result = await lastValueFrom(this.apiConection.getColumnsApelidoFromPG_beta(this.layerId));
  ...
  removeDroneTile(id: string) {
    ...
    if (!this.asLayer) {
      ...
      this.geoservice.removeLayerFromMapAndSidebar({id: Number(id), layer: layer})
    }
  }
