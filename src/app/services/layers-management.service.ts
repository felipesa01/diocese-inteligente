...
import { ApisConectionService, responseGSDescribeFeatureType } from "./apis-conection.service";
...
export class LayersManagementService {
  ...
  layerCatalog: layerCatalogItem[] = []

  foraniasLayer: VectorLayer<VectorSource> = new VectorLayer();
  igrejasLayer: VectorLayer<VectorSource> = new VectorLayer();
  municipiosLayer: VectorLayer<VectorSource> = new VectorLayer();
  quadrasLayer: VectorLayer<VectorSource> = new VectorLayer();
  quadrasSelectedLayer: VectorLayer<VectorSource> = new VectorLayer();

  constructor(private apiConection: ApisConectionService, public stylesMng: stylesMngService, private http: HttpClient, private auth: AuthService) {

    this.http.get('./assets/vetores/foranias.json').pipe(take(1)).subscribe(e => {
      let features: Feature[] = new GeoJSON({ dataProjection: "EPSG:4674", featureProjection: 'EPSG:4326', }).readFeatures(e);
      this.foraniasLayer.setSource(new VectorSource({
        features: features,
      }));
      const properties_ = {
        'name': 'Foranias',
        ...
        'feat_apelido': 'FORANIA',
        'filtros': ['gid'],
        'estilo_geoserver': '',
        'tela_gsu': '',
        'telas_gsu': ''
      }
      this.foraniasLayer.setProperties(properties_);
      this.foraniasLayer.setStyle(this.stylesMng.styleForanias);
    });
    ...
    this.http.get('./assets/vetores/municipios.json').pipe(take(1)).subscribe(e => {
      let features: Feature[] = new GeoJSON({ dataProjection: "EPSG:4674", featureProjection: 'EPSG:4326' }).readFeatures(e);
      this.municipiosLayer.setSource(new VectorSource({
        features: features,
      }));
      const properties_ = {
        'name': 'Município',
      ...
      this.municipiosLayer.setProperties(properties_);
      this.municipiosLayer.setStyle(this.stylesMng.styleMunicipios)
    });

    this.http.get('./assets/vetores/quadras.json').pipe(take(1)).subscribe(e => {
      let features: Feature[] = new GeoJSON({ dataProjection: "EPSG:4674", featureProjection: 'EPSG:4326', }).readFeatures(e);
      this.quadrasLayer.setSource(new VectorSource({
        features: features,
      }));
      const properties_ = {
        'name': 'Quadras',
        ...
        'fk_name': undefined,
        'cluster_mode': undefined,
        'feat_apelido': 'Responsabi',
        'filtros': ['gid'],
        'estilo_geoserver': '',
        'tela_gsu': '',
        'telas_gsu': ''
      }
      this.quadrasLayer.setProperties(properties_);
      // this.quadrasLayer.setStyle(this.stylesMng.styleForanias);
    });
  };
  ...
  customLoader(tileOrImage, src) {
    const client = new XMLHttpRequest();

    client.open('GET', src);
    client.responseType = 'arraybuffer';
    client.setRequestHeader("Authorization", `Bearer ${this.auth.tokenGSU}`);
    ...
    client.onload = function () {
      ...
      tileOrImage.getImage().src = imageUrl;
    };

    client.send();
  }

  createSingleWMSSource(layer: string, style: string) {
    // Depois melhorar isso aqui. Caso o nome do estilo seja escrito errado no banco (coluna estilo_geoserver) a camada não será exibida. 
    // Para corrigir, avaliar se o estilo de fato existe a partir da requisição rest ./geoserver/rest/workspaces/pmspwebgeo/styles/NOMEDOESTILO.sld?responseType=text
    var params;
    if (style) {
      params = { 'LAYERS': layer, 'STYLES': style }
    }
    else {
      params = { 'LAYERS': layer }
    }

    var WMSTileSource = new ImageWMS({
  ...
  createVectorImage(layerItem: layerCatalogItem, rotulo: boolean = false) {
    ...
    const properties_ = {
      ...
      'filtros': ['gid'],
      'estilo_geoserver': layerItem.estilo_geoserver,
      'tela_gsu': layerItem.tela_gsu,
      'telas_gsu': layerItem.telas_gsu
    }

    const tileLayer = new ImageLayer({
      source: this.createSingleWMSSource(layerItem.fonte, layerItem.estilo_geoserver),
  ...
  createWMSTiledSource(layer: string) {
    ...
    var WMSTileSource = new TileWMS({
      ...
      transition: 0,
      tileGrid: tileGrid,
      tileLoadFunction: this.customLoader.bind(this)
    });
  ...
  async getLayersFromPG(id?: number): Promise<ImageLayer<ImageWMS>[]> {
    var layerList = [];

    // Fazer aqui a requisição para o banco das informações da tabela "layer_catalog"
    if (!id) {
      var layerCatalog = await lastValueFrom(this.apiConection.getLayerCatalog_beta());
    }
    else {
      var layerCatalog = [await lastValueFrom(this.apiConection.getLayerInfo(id))] as any[];
    }
    this.layerCatalog = layerCatalog as layerCatalogItem[]
    ...
    layerCatalog.forEach(layerItem => {
      ...
      if (layerItem['tipo'].toLowerCase() === 'wms_geoserver') {
        ...
        var vectorLayer = this.createVectorImage(layerItem, layerItem['rotulo']);
  ...
  getGeomTypeFromSource(layerSource: string): Observable<responseGSDescribeFeatureType> {
    return this.apiConection.getAttributesTypeGS(layerSource)
  }
