...
export class InfoSearchFilterService {
  ...
  async emptyData_beta(layerId: number, layerSource: string, keys: { [key: string]: { apelido: string, mapavalor: boolean, valor: string | number } }, typeMode: string) {
    ...
    resultType.featureTypes[0].properties.forEach(e => {
      if (Object.keys(keys).includes(e.name)) {
        if (typeMode == 'filter' && filtersControlKeys.includes(layerId.toString())) {
          var filterFromStorage = this.geoservice.layersFiltersControl[layerId][e.name];
          emptyData[e.name] = this.formBuilder.group({ value: filterFromStorage.value, type: [e.localType], mode: filterFromStorage.mode, ismap: keys[e.name].mapavalor });
        }
        else if (typeMode == 'edit') {
          emptyData[e.name] = this.formBuilder.group({ value: { main: keys[e.name].valor, second: null }, type: [e.localType], mode: [0], ismap: keys[e.name].mapavalor });
        }
        else emptyData[e.name] = this.formBuilder.group({ value: { main: null, second: null }, type: [e.localType], mode: [0], ismap: keys[e.name].mapavalor });
  ...
  getIsSelect(layerSource: string, attribute: string) {
  ...
  async applySearch(layerSource: string, cqlStr: string, data: mappingResultObject, formatedKeys: {}) {
    ...
    var result = await lastValueFrom(this.apiConection.doSearchFromCQLFilter(layerSource, cqlStr));
